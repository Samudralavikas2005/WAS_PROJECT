...
// CRITICAL SECURITY CONTROL: Retrieve trusted product prices directly from database
$stmt = $pdo->prepare("
                                ...
                                <tr>
                                    <td>
                                        <div class="d-flex align-items-center gap-3">
                                            <img src="<?= APP_URL ?>/<?= e($item['image_url']) ?>" alt="" width="40" height="40" class="rounded bg-dark p-1">
                                            <div>
                                                <strong class="text-white d-block"><?= e($item['name']) ?></strong>
                                    ...
                                    <td class="text-center">
                                        <form action="<?= APP_URL ?>/cart/update.php" method="POST" class="d-inline-flex align-items-center gap-2">
                                            <?= csrf_field() ?>
                                            <input type="hidden" name="cart_id" value="<?= (int)$item['cart_id'] ?>">
                                            <input type="number" name="quantity" class="form-control form-control-custom form-control-sm text-center" value="<?= (int)$item['quantity'] ?>" min="1" max="<?= (int)$item['stock'] ?>" style="width: 70px;" onchange="this.form.submit()">
                                        </form>
                                    </td>
                                    <td class="text-end font-monospace text-info font-weight-bold">₹<?= number_format($itemSubtotal, 2) ?></td>
                                    <td class="text-end">
                                        <form action="<?= APP_URL ?>/cart/remove.php" method="POST" class="d-inline">
                                            <?= csrf_field() ?>
                                            <input type="hidden" name="cart_id" value="<?= (int)$item['cart_id'] ?>">
