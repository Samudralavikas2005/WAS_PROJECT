...
// Enforce strict Server-Side Admin Role Check (RBAC Protection)
require_admin();

$pdo = getDBConnection();

// Fetch summary metrics using Prepared Statements
$productCount = $pdo->query("SELECT COUNT(*) FROM products")->fetchColumn();
$orderCount = $pdo->query("SELECT COUNT(*) FROM orders")->fetchColumn();
$totalRevenue = $pdo->query("SELECT SUM(total_amount) FROM orders")->fetchColumn() ?? 0;
$userCount = $pdo->query("SELECT COUNT(*) FROM users")->fetchColumn();
$lockedCount = $pdo->query("SELECT COUNT(*) FROM login_attempts WHERE locked_until > NOW()")->fetchColumn();
...
<div class="row gy-4">
    ...
    <div class="col-md-6">
        <div class="card card-glass p-4 h-100">
            ...
            <div class="mb-3">
                <span class="badge <?= $lockedCount > 0 ? 'bg-danger' : 'bg-success' ?>">
                    <?= (int)$lockedCount ?> Accounts currently locked out
                </span>
            </div>
            <div>
