...
$stmt = $pdo->prepare("SELECT id, name, description, price, stock, category, image_url, created_at FROM products WHERE id = ?");
...
$revStmt = $pdo->prepare("SELECT r.rating, r.review_text, r.created_at, u.name as reviewer_name FROM reviews r JOIN users u ON r.user_id = u.id WHERE r.product_id = ? ORDER BY r.id DESC");
...
<div class="row gy-4 mb-5">
    ...
    <div class="col-md-5">
        <div class="card card-glass p-5 text-center">
            <img src="<?= APP_URL ?>/<?= e($product['image_url']) ?>" alt="<?= e($product['name']) ?>" class="img-fluid style-product-img mb-3" style="max-height: 250px;">
        </div>
    </div>
    ...
    <div class="col-md-7">
        <div class="card card-glass p-4 h-100 d-flex flex-column justify-content-between">
            <div>
                <span class="badge bg-dark text-info border border-info mb-2"><?= e($product['category']) ?></span>
                <h2 class="text-white font-weight-bold mb-3"><?= e($product['name']) ?></h2>
                <div class="product-price mb-3">₹<?= number_format($product['price'], 2) ?></div>
                <p class="text-muted lead fs-6 mb-4"><?= e($product['description']) ?></p>
            ...
            <?php if ($product['stock'] > 0): ?>
                <form action="<?= APP_URL ?>/cart/add.php" method="POST" class="mt-4 pt-3 border-top border-secondary">
                    ...
                    <!-- Note: Notice that price is NOT sent in form input to prevent Checkout Price Tampering -->
...
<!-- Customer Reviews Section (XSS Demonstration) -->
            ...
            <?php foreach ($reviews as $rev): ?>
                <div class="p-3 rounded bg-dark border border-secondary">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <strong class="text-white"><?= e($rev['reviewer_name']) ?></strong>
                        <span class="text-warning small"><?= str_repeat('⭐', (int)$rev['rating']) ?></span>
                    </div>
                    <!-- CRITICAL XSS PROTECTION: Review content rendered safely with htmlspecialchars -->
                    <p class="text-light mb-1 small"><?= e($rev['review_text']) ?></p>
                    <small class="text-muted font-monospace" style="font-size: 0.75rem;"><?= e($rev['created_at']) ?></small>
                </div>
            <?php endforeach; ?>
        </div>
    <?php endif; ?>
