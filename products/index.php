...
$catStmt = $pdo->prepare("SELECT DISTINCT category FROM products ORDER BY category ASC");
$catStmt->execute();
...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
    ...
    <form action="<?= APP_URL ?>/products/index.php" method="GET" class="d-flex gap-2">
        ...
        <?php endif; ?>
        <input type="text" name="search" class="form-control form-control-custom form-control-sm" placeholder="Search products..." value="<?= e($search) ?>">
            ...
            <div class="col-md-4">
                <div class="card card-glass product-card">
                    <div class="product-img-wrapper">
                        <img src="<?= APP_URL ?>/<?= e($product['image_url']) ?>" alt="<?= e($product['name']) ?>" class="product-img">
                    </div>
                    <div class="card-body d-flex flex-column justify-content-between p-4">
                        <div>
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <span class="badge bg-dark text-info border border-info"><?= e($product['category']) ?></span>
                            ...
                            <h5 class="card-title text-white font-weight-bold mb-2"><?= e($product['name']) ?></h5>
                            <p class="card-text text-description mb-3"><?= e(mb_strimwidth($product['description'], 0, 95, '...')) ?></p>
                        </div>
